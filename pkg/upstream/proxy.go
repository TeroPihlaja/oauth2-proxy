...
// multiple upstreams.
func NewProxy(upstreams options.Upstreams, auth hmacauth.HmacAuth, errorHandler ProxyErrorHandler) (http.Handler, error) {
	...
	for _, upstream := range upstreams {
		...
		case httpScheme, httpsScheme:
			m.registerHTTPUpstreamProxy(upstream, u, auth, errorHandler)
		default:
...
func (m *multiUpstreamProxy) registerStaticResponseHandler(upstream options.Upstream) {
	m.serveMux.Handle(upstream.Path, newStaticResponseHandler(upstream.StaticCode))
}
...
func (m *multiUpstreamProxy) registerHTTPUpstreamProxy(upstream options.Upstream, u *url.URL, auth hmacauth.HmacAuth, errorHandler ProxyErrorHandler) {
	logger.Printf("mapping path %q => upstream %q", upstream.Path, upstream.URI)
	m.serveMux.Handle(upstream.Path, newHTTPUpstreamProxy(upstream, u, auth, errorHandler))
}
